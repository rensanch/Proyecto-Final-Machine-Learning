---
title: "Machine Learning in Genomics - Proyecto Final"
...
#### **Accuracy**: proporción de predicciones correctas. Proporción de cuántas veces el modelo predijo correctamente. Esperamos que el modelo esté haciendo buenas predicciones cuando toma valores muy cercanos a 1.
    ...
    a. Lasso Model: el valor de L1 incia en ~1e-08 es el ~54% de sus predicciones son correctas,
    y se mantiene así hasta que L1 alcanza un valor de 1e-02, cuando comienza a crecer la
    proporción de predicciones correctas, alcanzando un máximo en un 75% de predicciones correctas, 
    después de esto, disminuye drásticamente.
    
    b. Ridge Model: durante la primera parte de la gráfica, tenemos una proporción del 0.68 de 
    predicciones correctas, hasta que llega a poco después de 1e-0.1, donde comienza a disminuir la 
    proporción, llegando a valores de 0.5.
    ...
    proporción de predicciones correctas (~0.68), pero también es la que no tiene regularización
...
####  **Brier class**: mide la calidad de las predicciones. Compara las probabilidades predichas por el modelo con el valor real. Esperamos que este valor sea muy cercano a 0, esto nos indicaría que el modelo está haciendo un buen trabajo.
    ...
    a. Lasso Model: el valor del Brier Score inicia en ~0.45, que lo consideramos alto. Este 
    valor se mantiene constante hasta que L1 toma valores de ~1e-0.2, donde hay una caída 
    en este valor, llegando a un coeficiente de ~ 0.25.
    
    b. Ridge Model: iniciamos con un valor de ~0.22 y se mantiene constante hasta poco después
    del ~1e-0.1, donde comienza a crecer la curva, lo que indica que las predicciones son  malas.
    
    c. Elastic Net: por otro lado, el modelo que tiene una mejor calidad en las predicciones
    (Brier Class pequeño) es aquel que tiene una proporción de L1 = 1, con un valor en esta
    métrica de ~0.4, con un mínimo que llega a ~0.23.
...
####  **ROC-AUC**: esta curva nos muestra la relación que hay entre los verdaderos positivos y los falsos positivos. Valores cercanos a 1 indican un muy buen modelo, mientras que valores cercanos 0, son señalan de un bajo rendimiento en el modedlo.
    ...
    a. Lasso Model: en este modelo, iniciamos con un valor de 0.6, el cual se mantiene 
    conforme crece L1, hasta llegar a ~1e-0.2, donde, al igual que en las otras métricas, toma
    un mejor valor (0.75).
    
    b. Ridge Model: el valor bajo la curva es alto medianamente bueno (0.71), hasta que llegamos 
    a poco después de 1e-0.1, donde comienza a dismnuir el  valor de ROC-AUC, lo cual no es lo 
    ideal. En conclusión, esperamos que un buen modelo esté por debajo de un valor de L2 de ~1e-0.1.
    
    c. Elastic Net: por otro lado, para esta métrica, el mejor modelo es aquel que tiene la
    proporción de L1 de 0, seguido del modelo de proporción L1 = 0.25. En este caso, resulta
...
kappa_values <- tibble(
  Model = c("Ridge", "Random Forest", "Decision Tree"),
  Kappa = c(
    metrics_result_ridge %>% filter(.metric == "kap") %>% pull(.estimate),
